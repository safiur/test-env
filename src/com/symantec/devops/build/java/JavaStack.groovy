...
def checkUnusedRoutes()
{
   try {
      wrap([$class: 'AnsiColorBuildWrapper']) {
        println "\u001B[32mINFO => Checking unused routes for ruby on rails with traceroute, please wait..."
        sh "rake traceroute"
      }
   }
   catch (Exception caughtException) {
      wrap([$class: 'AnsiColorBuildWrapper']) {
        println "\u001B[41mERROR => failed to get the unused routes for ruby on rails, exiting..."
        currentBuild.result = 'FAILED'
        throw caughtException
      }
   }
